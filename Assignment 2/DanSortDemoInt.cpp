...
// populates the queue
    int vectorNumbers[] = {60, 13, 15, 1, 50, 43, 81, 66, 5, 9, 33, 31, 38, 45,
                           72, 74, 55, 3, 20, 84, 90, 64, 62, 11, 30, 22, 18, 17, 6, 88, 47, 71, 68, 37, 19,
                           85, 52, 57, 59, 77, 4, 63, 87, 54, 26, 21, 35, 2, 61, 73};
    int numValues = sizeof(vectorNumbers)/sizeof(vectorNumbers[0]);
    cout << numValues << endl;
    cout << "This is a demo for DanSort for integers, which is a made up thing."
         << endl << endl;
...
// populates the stacks
    ...
    {
        counter = 0;
        while ((!myStacks[counter].isEmpty()) && (myStacks[counter].peek() <
                                                  myQueue.front())) {counter++; numComparisons++;}
...
// finds the minimum value among the stacks and removes it, iteratively.
    ...
    {int min = INT_MAX, minStack = -1;
        for (int i=0; i<numStacks; i++)
        {
            ...
            {
                numComparisons++;
                if (myStacks[i].peek() < min) {min = myStacks[i].peek();
                    minStack = i;}
            }
        }
